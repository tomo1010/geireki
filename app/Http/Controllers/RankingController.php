...
//dd($youtubes);
    ...
    */
    ...
    public function tag()
    {
        ...
        $tags = Tag::all()->groupBy('category'); //タグを取得        
        $tagNames = Tag::all()->groupBy('category'); //タグ名取得用                
        ...
        foreach($tags as $category=>$tag){
            ...
            foreach($tag as $tagId){
                ...
                $tagCounts[$category][] = Entertainer::withCount(['tags' => function ($query)  use ($tagId){  //芸人のtag件数ごと上位10件取得
                        $query->where('tag_id', $tagId->id);
                }])->orderBy('tags_count', 'desc')->take(10)->get();
